Shader "Water/Custom/WaterProDoubleFoamTexture" 
{
	/* 
	...
	Properties 
	{
		...
		_FoamOffset("Foam offset", float) = 0
		_FoamFade("Foam fade", float) = 4
		_ShoreLineOffset("Shore line offset", float) = 0
		_ShoreLineFade("Shore line fade", float) = 10
	}
	SubShader 
	{
		// Subshader Tags
		...
		////// Uniform user variable definition
			...
			uniform sampler2D _ShoreLineTex;			// foam texture
			uniform float _ShoreLineOffset;				// shore line offset
			uniform float _ShoreLineFade;				// shore line fading
		...
		////// Shader functions
			...
			float4 frag(Vert2FragPro fragIn) : SV_Target
			{
				...
				foam += GetFoam(_ShoreLineTex, fragIn.uv_Environment.zw, depthLevel, _ShoreLineOffset, _ShoreLineFade);
