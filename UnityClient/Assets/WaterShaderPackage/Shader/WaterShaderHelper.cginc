...
////// Vertex function
	Vert2FragPro HandleVertexInput(VertexInputPro vertIn, float4 mainTexConfig, float4 normalTexConfig, float4 uvOffset, float foamScale, float foamSpeed, float4 _Waves, float _WaveHeight)
	{
		Vert2FragPro output;
		UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(output);
		float4 pos = mul(unity_ObjectToWorld, vertIn.vertex);
		float x = sin(pos.x*_Waves.x*_Time.x) + cos(pos.x*_Waves.y*_Time.x);
		float y = sin(pos.z*_Waves.z*_Time.x) + cos(pos.z*_Waves.w*_Time.x);

		output.pos = UnityObjectToClipPos(vertIn.vertex + _WaveHeight * (x + y));
		output.posWorld = float4(pos.x, pos.y + _WaveHeight * (x + y), pos.zw);
		...
		float2 foamUv = (vertIn.uv_NormalTex * foamScale + normalize(uvOffset.xy)*foamSpeed*_Time.x);
	...
	Vert2FragPro HandleVertexInput(VertexInputPro vertIn, float4 mainTexConfig, float4 normalTexConfig, float4 heightMaptexConfig, float4 uvOffset, float foamScale, float4 _Waves, float _WaveHeight)
	{
		Vert2FragPro output;
		output = HandleVertexInput(vertIn, mainTexConfig, normalTexConfig, uvOffset, foamScale, 1, _Waves, _WaveHeight);
