Shader "Water/WaterLWRP" 
{
	...
	Properties 
	{
		...
		_NormalMainTexStrength("Normal on mainTex", Range(0,2)) = 1
		...
		_Waves("XSin, XCos, YSin,YCos", Vector) = (0.5,0.4,0.1,0.098)
	...
	SubShader 
	{
		...
		////// Uniform user variable definition
			...
			uniform float _NormalMainTexStrength;				// normal map multiplier
			...
			uniform float _FoamSpeed;					// foam texture scale
		...
		////// Shader functions
			...
			{
				Vert2FragPro output;
				output = HandleVertexInput(vertIn, _MainTex_ST, _NormalTex_ST, _UVOffset, _FoamScale, _FoamSpeed, _Waves, _WaveHeight);
				return(output);
			}
			...
			float4 frag(Vert2FragPro fragIn) : SV_Target
			{
				...
				float3 mainColor = GetMainColor(_MainTex, fragIn.uv_Tex+ normalDir* _NormalMainTexStrength);
